...
## Integration with REFLEKT Console

The Credit Engine is designed to work seamlessly with the main REFLEKT Console application.
...
class CreditEngineAPI {
  constructor(apiKey) {
    this.apiKey = apiKey;
    this.baseUrl = 'https://credit-api.reflekt.sh';
  }
...
class CreditEngineClient:
    def __init__(self, api_key: str, base_url: str = "https://credit-api.reflekt.sh"):
...
class CreditEngineService {
  private apiKey: string;
  private baseUrl = 'https://credit-api.reflekt.sh';
...
class CreditEngineClient {
  final String apiKey;
  final String baseUrl = 'https://credit-api.reflekt.sh';
...
const client = new ApolloClient({
  uri: 'https://credit-api.reflekt.sh/graphql',
...
// Usage
import { CreditEngineSDK } from '@reflekt/credit-engine-sdk';
