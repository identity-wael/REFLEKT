---
title: API Reference
description: Complete API documentation for REFLEKT Console
---
...
Complete REST API reference for REFLEKT Console. The API provides endpoints for chat interactions, model management, avatar sessions, voice conversations, and file handling powered by GPT-4o and multiple AI providers.
...
<Tabs items={['Request', 'Response', 'Examples']}>
  ...
  <Tabs.Tab>
    ```json
    {
      "response": "REFLEKT Console is a multi-model chat platform powered by GPT-4o...",
      "model": "gpt-4o",
      "usage": {
        "promptTokens": 42,
...
{
  "fileId": "file_xyz123",
  "url": "https://storage.reflekt.sh/files/file_xyz123",
  "name": "document.pdf",
  "size": 1024000,
...
{
  "imageUrl": "https://storage.reflekt.sh/files/image.jpg",
...
```javascript
const ws = new WebSocket('wss://api.reflekt.sh/realtime');
...
```typescript
import { createClient } from '@reflekt/sdk';

const client = createClient({
  apiKey: process.env.EARNA_API_KEY,
  baseUrl: 'https://api.reflekt.sh/v1'
});
...
client = reflekt.Client(
    api_key=os.environ["EARNA_API_KEY"],
    base_url="https://api.reflekt.sh/v1"
)
...
```bash
# Send a chat message
curl -X POST https://api.reflekt.sh/v1/api/chat \
...
# Get chat history
curl -X GET "https://api.reflekt.sh/v1/api/chat/history?chatId=chat_abc123" \
  -H "Authorization: Bearer $API_KEY"
```
...
- **JavaScript/TypeScript**: `pnpm add @reflekt/sdk`
- **Python**: `pip install reflekt`
- **Go**: `go get github.com/reflekt/go-sdk`
- **Ruby**: `gem install reflekt`

<Callout type="info">
For detailed SDK documentation, visit [sdk.reflekt.sh](https://sdk.reflekt.sh)
</Callout>
...
For API support:
- Documentation: [docs.reflekt.sh](https://docs.reflekt.sh)
- Status: [status.reflekt.sh](https://status.reflekt.sh)
- Support: support@reflekt.sh
