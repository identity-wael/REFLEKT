...
Temporal provides durable workflow execution for the REFLEKT platform, ensuring reliable processing of financial operations, data synchronization, and complex business logic. Our Temporal deployment handles mission-critical workflows with guaranteed completion and full observability.
...
spec:
  ...
  template:
    spec:
      containers:
      - name: worker
        image: gcr.io/production-reflekt-ai/temporal-worker:latest
        env:
        - name: TEMPORAL_HOST
