...
Complete reference for all environment variables required across the REFLEKT monorepo projects.
...
```bash
# Vercel
NEXT_PUBLIC_VERCEL_URL=https://console.reflekt.sh
...
```bash
# Domain
NEXT_PUBLIC_APP_URL=https://reflekt.sh

# SEO
NEXT_PUBLIC_SITE_NAME="REFLEKT"
```
...
# NEXTAUTH_URL=https://credit.reflekt.sh  # Production
```
...
GCP_PROJECT_ID=production-reflekt-ai
...
# CloudSQL (Temporal)
CLOUDSQL_CONNECTION_NAME=production-reflekt-ai:us-central1:temporal-production
CLOUDSQL_DATABASE=temporal
CLOUDSQL_DATABASE_VISIBILITY=temporal_visibility
CLOUDSQL_PRIVATE_IP=10.147.0.2

# Workload Identity
WIF_SERVICE_ACCOUNT=[service-account]@production-reflekt-ai.iam.gserviceaccount.com
